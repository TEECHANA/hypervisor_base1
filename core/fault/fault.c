...
#include "../../core/vm/vm.h"          /* VSE Phase 5: full vm_t for ->id */
...
void fault_dabt(void *regs, u64 esr)
{
    ...
    if (dfsc >= 0x10 && dfsc <= 0x13) {
        ...
        /* VSE Phase 5: report to trust engine */
        {
            ...
            u32 vmid = (g_current_vcpu[0] && g_current_vcpu[0]->vm)
                       ? g_current_vcpu[0]->vm->id : 0u;
            bool is_wr = (bool)((esr >> 6) & 1u);
            fdetect_mem_fault(vmid, ipa, esr, is_wr);
        }

        return;
     }

    /* VSE Phase 5: report unhandled DABT to trust engine */
    {
        extern vcpu_t *g_current_vcpu[];
        u32 vmid = (g_current_vcpu[0] && g_current_vcpu[0]->vm)
                   ? g_current_vcpu[0]->vm->id : 0u;
        bool is_wr = (bool)((esr >> 6) & 1u);
        fdetect_mem_fault(vmid, far, esr, is_wr);
    }
