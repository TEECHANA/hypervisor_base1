...
def pw_define(password, master_key=DEV_MASTER_KEY):
    """Single-line, no-space brace list for -DVSE_PW_VERIFIER=... at build time,
    e.g. {0x21,0x34,...}. Suitable to pass through make/shell without quoting
    issues (no spaces)."""
    d = pwverify_bytes(password, master_key)
    return "{" + ",".join(f"0x{b:02x}" for b in d) + "}"
...
def pw_header(password, master_key=DEV_MASTER_KEY, is_dev=True):
    ...
    # the plaintext is never stored). is_dev tracks dev-key vs device-key.
    ...
    return (
        ...
        " *\n"
        f" * Derived with the {keysrc}. Generated by provision_password.sh; the\n"
        " * committed repo default is instead fail-closed (an #error, no password).\n"
        " *\n"
...
def main():
    ...
    p.add_argument("--pw-define", type=str, metavar="PASSWORD",
                   help="Print single-line {0x..,..} for -DVSE_PW_VERIFIER=... for PASSWORD")
    ...
    if args.pw_define is not None:
        print(pw_define(args.pw_define, master_key))
        return
